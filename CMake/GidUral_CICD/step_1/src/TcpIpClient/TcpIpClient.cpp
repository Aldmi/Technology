...
[[noreturn]] void TcpIpClient::ConnectAndStartListen_Blocked()
{
    ...
    do
    {
        cout<<"Try Connect to...." <<_ipAddress<<":"<<_portNumber<<endl;
        ...
        else
        {
            ...
            cout << "Connect OK to " << _ipAddress <<":"<< _portNumber<<endl;
        }
...
//Отправить буфер в сокет
bool TcpIpClient::SendRequest(Byte* buf, size_t size)
{
    auto n = write(sockfd, buf, size);
    return n == size;
}