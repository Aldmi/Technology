...
private:
    ...
    KafkaProducer(RdKafka::Producer *producerP){
...
public:
    ...
    static KafkaProducer* CreteProducer(string brokers, ExampleDeliveryReportCb &deliveryReportCb, std::string &errstr){
...
//        }
        ...
        auto instance= new KafkaProducer(producer);
        return instance;
    }


    void SendMessage(string topicName, string message, map<string, string> mapHeaders)
    {
        RdKafka::Headers *headers = RdKafka::Headers::create();
        for (const auto& [key, value] : mapHeaders){
        ...
        if (err != RdKafka::ERR_NO_ERROR) {
            std::cerr << "% Failed to produce to topic " << topicName << ": "
